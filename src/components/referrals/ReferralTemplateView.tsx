...
export default async function ReferralTemplateView({ companyId, pipelineId }: { companyId: number, pipelineId: string }) {
        ...
        <ul className="space-y-2">
          {items.map((a) => (
            <li key={a.attribute_name} className="flex items-center gap-3 text-sm">
              ...
              />
              <span className={a.fit_color === 'grey' ? 'text-zinc-400' : 'text-zinc-900 dark:text-zinc-100'}>
                {a.attribute_name}
              </span>
            </li>
          ))}
          ...
          <div className="space-y-6">
            {deals.length === 0 && (
              <div className="text-sm text-zinc-500">
                No jobs found for this company in pipeline ID “{pipelineId}”.
              </div>
            )}
            {deals.map((d) => {
              ...
              const sortByColor = (arr: any[]) =>
                arr.sort((x, y) => (priority[x.fit_color] ?? 3) - (priority[y.fit_color] ?? 3))
              const byCat = {
